...
contract StakeManager is IStakeManager, Ownable, ERC165 {
    ...
    string public override versionSM =
        "3.0.0-beta.3+opengsn.stakemanager.istakemanager";
    ...
    {
        bool isHubAuthorized = authorizedHubs[relayManager][msg.sender]
            .removalTime == type(uint256).max;
    ...
    /// @inheritdoc IStakeManager
    function getMaxUnstakeDelay() external view override returns (uint256) {
    ...
    function stakeForRelayManager(
        ...
        uint256 amount
    ) external override relayOwnerOnly(relayManager) {
        require(
    ...
    /// @inheritdoc IStakeManager
    function authorizeHubByManager(
        address relayHub
    ...
    /// @inheritdoc IStakeManager
    function unauthorizeHubByManager(
        address relayHub
    ) external override managerOnly {
    ...
    function penalizeRelayManager(
        ...
        uint256 removalTime = authorizedHubs[relayManager][msg.sender]
            .removalTime;
    ...
    function isRelayEscheatable(
        ...
        return
            stakeInfo.abandonedTime != 0 &&
            stakeInfo.abandonedTime +
                abandonedRelayServerConfig.escheatmentDelay <
            block.timestamp;
    }

    /// @inheritdoc IStakeManager
    function markRelayAbandoned(
        address relayManager
        ...
        require(
            info.keepaliveTime + abandonedRelayServerConfig.abandonmentDelay <
                block.timestamp,
            "relay manager was alive recently"
        );
    ...
    /// @inheritdoc IStakeManager
    function escheatAbandonedRelayStake(
        address relayManager
    ) external override onlyOwner {
        StakeInfo storage info = stakes[relayManager];
        require(
            isRelayEscheatable(relayManager),
        ...
        emit AbandonedRelayManagerStakeEscheated(
            relayManager,
            msg.sender,
            info.token,
            amount
    ...
    function updateRelayKeepaliveTime(address relayManager) external override {
        ...
        require(
            isHubAuthorized || isRelayOwner,
            "must be called by owner or hub"
        );
